...
custom_palette <- c(
  ...
  "grey1", "grey10", "grey25", "grey50", "grey75", "grey100"
)
...
drives_by_cap <- dat[,list(drives=sum(n_unique)), by=capacity_tb][order(-drives),]
caps_to_keep <- drives_by_cap[drives > 999, capacity_tb]
dat <- dat[capacity_tb %in% caps_to_keep,]
drive_dates <- drive_dates[capacity_tb %in% caps_to_keep,]

# Sort model factors by survival rates
model_levels = dat[,model[order(-surv_5yr_lower, -surv_5yr, surv_5yr_upper)]]
dat[,model := factor(model, levels=model_levels, ordered=T)]
drive_dates[,model := factor(model, levels=model_levels, ordered=T)]

# Sort capacity factors by size
capacity_levels = dat[,rev(sort(unique(capacity_tb)))]
dat[,capacity_tb := factor(capacity_tb, levels=capacity_levels, ordered=T)]
drive_dates[,capacity_tb := factor(capacity_tb, levels=capacity_levels, ordered=T)]
...
* **years_97pct** is the expected number of years 97% of these drives will last
...
# The cox model orders levels alphabetically
# We need to map the desired order to the alpha order
alpha_order <- cox_model$xlevels$model
alpha_order <- alpha_order[alpha_order %in% model_list]
desired_order <- best_drive_by_size[order(capacity_tb), as.character(model)]

named_palette <- setNames(custom_palette, desired_order)
reordered_palette <- named_palette[alpha_order]
reordered_palette <- unname(reordered_palette)
...
colnames(plot_model$surv) <- stringi::stri_pad_left(order(best_drive_by_size[['capacity_tb']]), 2, '0')
