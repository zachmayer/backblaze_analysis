...
# Calculate poisson 95% percentile of coun
vector_poission <- function(x) sapply(x, function(a) poisson.test(a)$conf.int[2])
all_data[,yes_95_ci := vector_poission(Yes)]

# Convert poisson counts to yearly counts
days_to_year <- 365.2425
all_data[,days_observed := Yes + No]
all_data[,annual_failure_count := Yes * (days_to_year / days_observed)]
all_data[,annual_failure_count_95 := yes_95_ci * (days_to_year / days_observed)]

# Convert yearly counts to yearly failure rates
all_data[,annual_failure_rate := 1-exp(-annual_failure_count)]
all_data[,annual_failure_rate_95 := 1-exp(-annual_failure_count_95)]

# Order data
setorderv(all_data, c('annual_failure_rate_95', 'annual_failure_rate'))

# Round off for nice formatting
all_data[,annual_failure_rate := sprintf("%1.2f%%", 100*annual_failure_rate)]
all_data[,annual_failure_rate_95 := sprintf("%1.2f%%", 100*annual_failure_rate_95)]
