...
# Load data
dat = data.table::fread('results/survival.csv')[capacity_tb >= 2,]
drive_dates = data.table::fread('results/drive_dates.csv')[capacity_tb >= 2,]
...
# Order factors
levs = dat[,model[order(-surv_5yr_lower, -surv_5yr, surv_5yr_upper)]]
dat[,model := factor(model, levels=levs, ordered=T)]
drive_dates[,model := factor(model, levels=levs, ordered=T)]
...
# Setup factors -- think about this
levs = plot_dat[,rev(sort(unique(capacity_tb)))]
plot_dat[,capacity_tb := factor(capacity_tb, levels=levs)]
dat[,capacity_tb := factor(capacity_tb, levels=levs)]
```
...
sizes <- best_drive_by_size[,rev(sort(capacity_tb))]
newdata <- data.table::data.table(model=as.character(model_list[best_drive_by_size[,order(capacity_tb)]]))
plot_model <- survival::survfit(cox_model, newdata=newdata)
out <- survminer::ggsurvplot(
