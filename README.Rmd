...
# Only keep models were at least 250 drives made it to one year
data_raw[,count_one_year := sum(drive_days>=days_to_year), by='model']
data_raw <- data_raw[count_one_year > 249,]
...
data_surv[,capacity_tb_round := round(capacity_tb)]
best_drives <- data_surv[,list(
  model = model[which.max(lower)],
  lower = max(lower)
), by=capacity_tb_round][order(-lower),]
best_drive_12 <- best_drives[capacity_tb_round==12,][1, model]
best_drive_4 <- best_drives[capacity_tb_round==4,][1, model]
```
...
So why do I recommend buying `r best_drive_12`?
...
Zooming in on the first year of a drive's life demonstrates that the `r rate_12` has a lower failure rate during this period.  Obviously the future beyond that is unknown, but I expect this lower failure rate to continue over the lifetime of the two drives:
...
dat <- data_raw[model %in% best_drives[,model],]
dat[,capacity_tb := factor(round(capacity_tb))]
model <- dat[,survfit(Surv(drive_days, failure) ~ capacity_tb)]

out <- ggsurvplot(
  model, data=dat, conf.int = T, ylim=c(.96, 1.0),
  palette = colors
  )
print(out)

out <- ggsurvplot(model, data=dat, fun='cumhaz', palette = colors, conf.int = T)
print(out)
```
...
Survival analysis is a little weird, becuase you don't observe the full distribution of your data.  This makes some traditional statistics impossible to calculate.
...
failure_rate_formatted <- sprintf("%1.2f%%", failure_rate)
knitr::kable(best_drives)
```

So at most, we've seen `r failure_rate_formatted` drives fail, which means we're at most seeing the `r failure_rate_formatted` percentile for drive survival time— we cannot make any inferences about percentiles higher than this, becuase **not enough drives have failed yet!**.  Interestingly, we can start to estimate a lower bound on survival time at a given quantile as we get close to it.

So, here is a survival table for the known quantiles for these 2 drives, along with upper and lower bounds on the estimated time-to-survival for that quantile:
...
#quantiles = seq(0, failure_rate, length=11)[-1]
quantiles = seq(0, 1, length=11)[-1]
data_quant <- data_raw[model %in% c(best_drive_12, best_drive_4), ]
data_quant <- data_quant[, c(
    list(capacity_tb=max(capacity_tb)),
    survival_quantile(drive_days, failure,  quantiles=quantiles)
  ),by='model']
knitr::kable(data_quant)

data_quant[,capacity_tb := factor(capacity_tb)]
plt <- ggplot(data_quant, aes(x=percentile, y=days, ymin=lower, ymax=upper, col=capacity_tb)) + 
  geom_point(na.rm=T) + 
  geom_errorbar(na.rm=T) + 
  ggtitle('Estimated survival days at different quantiles for 4TB vs 12TB drives') + 
  theme_tufte()
print(plt)
```
