...
```{r prices, echo=FALSE}
sprintf_vec <- function(fmt, ...) {
  args <- list(...)
  if (length(args) == 1 && is.vector(args[[1]])) {
    args <- as.list(args[[1]])
  }
  do.call(sprintf, c(fmt = fmt, args))
}

ebay_url <- function(x, text=x){
  x = rev(stringi::stri_split_fixed(x, ' ')[[1]])[1]
  sprintf_vec("[%s](https://www.ebay.com/sch/i.html?_from=R40&_nkw=%s&_sacat=0&LH_BIN=1&_sop=15&rt=nc&LH_ItemCondition=1000|1500)", text, x)
}
...
prices <- prices[,list(model, price, capacity_tb, surv_5yr_lower=surv_5yr_lower*100, cost_per_tb, cost_per_tb_5yr)]
best_model <- prices[1,]
best_model_name <- 
knitr::kable(prices, digits=1)
```

According to this analysis, the most cost effective drive is the `r best_model$model`, which costs `r best_model$price` and has a 5 year survival rate of `r sprintf("%1.1f%%", 100*best_model$surv_5yr_lower)`. This drive costs `r sprintf("$%1.2f", best_model$cost_per_tb)` per TB, and `r sprintf("$%1.2f", best_model$cost_per_tb_5yr)` to store 1 TB for 5 years.
