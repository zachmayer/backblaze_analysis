...
drive_dates[,model := factor(model, levels=model[order(-surv_5yr_lower, -surv_5yr, surv_5yr_upper)], ordered=T)]
...
plot_dat <- drive_dates[model %in% model_list,][capacity_tb >= 2,]
plot_dat[,capacity_tb := factor(capacity_tb, levels=rev(sort(unique(capacity_tb))))]
```
...
capacity_tb <- best_drive_by_size[,sort(capacity_tb)]
