...
poisson_results <- all_data[,pois.exact(Yes, (Yes+No)/days_to_year)]
...
knitr::kable(all_data[
  annual_drive_failure_rate_95 < .10,
  list(
    model, 
    capacity_tb, 
    Failures=Yes,
...
km_fit <- data_surv[model=='HGST HMS5C4040BLE640',survfit(Surv(time=N, event=failure) ~ 1)]

km_results <- data.table(summary(km_fit)$table)

km_results[, model := gsub('model=', '', names(km_fit$strata), fixed=T),]
km_results <- km_results[,list(
  model,
  days_to_fail_median=median,
  ci_95=`0.95LCL`
)]
km_results <- km_results[!is.na(ci_95),]
km_results <- km_results[order(-ci_95, -days_to_fail_median),]
km_results

cox <- coxph(Surv(N, failure) ~ model, data = data_surv)
```
